...
# Layout #
The first four sectors(2kb) of the disk are reserved for bootcode or whatever an OS wants to do with them. Next comes the FS\_DESCRIPTOR structure in the first sector after the reserved sector. Next comes the DS\_ENTRY blocks that define the directory structure. At the end of the first DS\_ENTRY blocks is a BLOCK\_ARRAY\_ENTRY that points to another section of DS\_ENTRIES that extend the directory structure (and so on with the next section of DS\_ENTRIES). Next, comes the FD\_ENTRIES that are organized much like the DS\_ENTRIES.
After all this comes the list of free blocks (Which also can be expanded like the DS\_ENTRIES). You can see their structure below.
